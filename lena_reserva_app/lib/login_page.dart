...
class _LoginPageState extends State<LoginPage> {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Inicie sesión para consultar las reservas.')),
          );
        }
      });
    }
  ...
  Future<void> submit() async {
    ...
    try {
      final result = await ApiService.login(emailController.text.trim(), passwordController.text);
      await auth.signIn(
        email: emailController.text.trim(),
        token: result['accessToken'].toString(),
        newRefreshToken: result['refreshToken']?.toString(),
      );
      if (mounted) Navigator.pushReplacementNamed(context, widget.redirectTo ?? '/inicio');
    } on ApiException catch (exception) {
      if (mounted) {
        setState(() => error = exception.statusCode == 401
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(
            ...
            children: [
              _NavButton(label: 'Login', active: widget.redirectTo != '/app/reservas', onPressed: () {}),
              _NavButton(label: 'Registro', onPressed: () => Navigator.pushNamed(context, '/registro')),
              _NavButton(
                label: 'Reservas',
                      ...
                      child: Column(
                        children: [
                          Container(
                            ...
                            child: const Icon(Icons.restaurant, color: AppColors.primary, size: 42),
                          ),
                          const SizedBox(height: 22),
                          const Text('Leña Reserva App', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w500)),
                          const SizedBox(height: 8),
                          const Text('Inicia sesión para continuar', style: TextStyle(color: Colors.black54)),
                          ...
                          TextFormField(
                            ...
                            validator: (value) => value == null || value.length < 4 ? 'La contraseña debe tener al menos 4 caracteres.' : null,
                          ),
                          if (error != null)
                            Padding(
